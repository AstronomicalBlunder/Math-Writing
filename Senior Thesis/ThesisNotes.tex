...
\end{equation*}
where $\omega$ is a unitary character with conductor $\pp^n$. Note the indices $s, n$ do not completely determine the character $\chi_{s, n}$, as there is some choice of $\omega$ to be made, but this notation will end up sufficing for the computations ahead.\\
...
\begin{align*}
    Z(\hat{f}, \chi^\lor_{s, 0}) &= \int_{\calo_F \setminus \set{0}} \hat{f}(x) \chi^\lor_{s, 0} \d{^*x}\\
    &= \vol(\pp^m, \dx) \sum_{k \geq 0} \int_{\calo_F^\times} \abs{\pi^k x}^{1-s} \d{^*x}\\
